...
def main():
    ...
    print("1 -  Running matrix generation (merge.py)...")
    subprocess.run(['python', matrix_script, args.input_csv], check=True)

    # 2) Discover the latest output files
    coords_pattern = os.path.join(script_dir, 'data/output/coordinates_*.csv')
    matrix_pattern = os.path.join(script_dir, 'data/output/matrix_*.csv')
    ...
    print("2 - Running PDF generation (generation-pdf.py)...")
    subprocess.run([
        'python', pdf_script,
