...
# Génération aléatoire des demandes des clients (poids des colis)
...
# Fonctions pour créer une solution valide
def split_route_for_trucks(cities, trucks, demands):
    ...
    # Affectation des villes aux camions
    for city in remaining_cities:
        # Choisir un camion au hasard
        selected_truck = random.choice(trucks)
        selected_truck.route.append(city)
    
    # Vérifier et ajuster les capacités
    for truck in trucks:
        # Calculer la charge totale
        total_load = sum(demands[city] for city in truck.route if city != DEPOT_INDEX)
        
        # Si la charge dépasse la capacité, retirer des villes jusqu'à ce que ce soit ok
...
def crossover(parent1, parent2):
    ...
    # Distribuer les villes restantes aux camions qui n'ont pas encore de route
